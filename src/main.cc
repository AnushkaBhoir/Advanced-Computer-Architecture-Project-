...
void swap_context(uint8_t swap_cpu_0, uint8_t swap_cpu_1)
{
	...
	//Since context-switch penalty is not included, this needs to be done, once it is added, fetch_stall will be 0 as branch mispredict penalty 
    //is only 20, while context-switch penalty is in miliseconds 
	ooo_cpu[swap_cpu_0].fetch_stall = 0;
	ooo_cpu[swap_cpu_1].fetch_stall = 0;

	cout << "FLUSHING TLB \n\n\n" << endl;
	//TLB flush
    //////////////////////////////////////////////////////////////////code to flush BTB on every context switch
	ooo_cpu[swap_cpu_0].BTB.flush_TlB();
	ooo_cpu[swap_cpu_1].BTB.flush_TLB();
    /////////////////////////////////////////////////////////////////
...
//	}
	...
	/*	
		...
		if(ooo_cpu[i].operating_index!=-1)
		{
			...
			if(ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[0]].ROB.occupancy==0 && ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[1]].ROB.occupancy==0 && 
            current_core_cycle[cs_file[ooo_cpu[i].operating_index].swap_cpu[0]] == current_core_cycle[cs_file[ooo_cpu[i].operating_index].swap_cpu[1]])
			{
				...
				cout <<"IMPLEMENT CONTEXT SWITCH on cpu " << i <<" cycle of cpu0" << current_core_cycle[cs_file[ooo_cpu[i].operating_index].swap_cpu[0]] << " CPU1 " << 
                current_core_cycle[cs_file[ooo_cpu[i].operating_index].swap_cpu[1]] << endl;
